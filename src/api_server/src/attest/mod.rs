...
#[cfg(any(feature = "native-as", feature = "native-as-no-verifier", feature = "native-as-hpcr-verifier"))]
mod native;
...
impl AttestVerifier {
    ...
    pub async fn new(kbs_config: &Config) -> Result<Self> {
        let attest_verifier: Arc<Mutex<dyn Attest>> = {
            cfg_if::cfg_if! {
                if #[cfg(any(feature = "native-as", feature = "native-as-no-verifier", feature = "native-as-hpcr-verifier"))] {
                    ...
                    compile_error!("Please enable at least one of the following features: `native-as`, `native-as-no-verifier`, `native-as-hpcr-verifier`, or `grpc-as` to continue.");
                }
            }
        };
